 ...
 * calculos.ts
 * ---------------------------------------------------------------------------
 * MOTOR DE CÁLCULO NUMÉRICO DETERMINÍSTICO do ProtocolFit.
 *
 * Aqui NÃO existe inteligência artificial: toda a matemática nutricional é
 * calculada por fórmulas científicas consagradas, garantindo resultado
 * IDÊNTICO para o mesmo perfil de entrada (determinismo), custo zero de
 * chamadas a LLMs e zero erro de cálculo.
 *
 * Fórmula base: Mifflin-St Jeor (1990), padrão ouro para estimar a taxa
 * metabólica basal (TMB).
 * ---------------------------------------------------------------------------
 */
import type { DistribuicaoMacros, Objetivo, Sexo } from '../tipos.js';
import { FAIXAS_ETARIAS, MINIMO_CALORIAS_POR_SEXO } from '../util/constantes.js';
...
const FATORES_DE_ATIVIDADE: Record<number, number> = {
  1: 1.2, // Sedentário: treina 1x por semana
  2: 1.375, // Levemente ativo: treina 2x por semana
  3: 1.55, // Moderadamente ativo: treina 3x por semana
  4: 1.725, // Muito ativo: treina 4x por semana
  5: 1.725, // Muito ativo: treina 5x por semana
  6: 1.9, // Extremamente ativo: treina 6x por semana
  7: 1.9, // Atleta: treina 7x por semana
};
...
const KCAL_POR_GRAMAS = {
  proteina: 4, // 1 g de proteína = 4 kcal
  carboidrato: 4, // 1 g de carboidrato = 4 kcal
  gordura: 9, // 1 g de gordura = 9 kcal
} as const;

/** Mililitros de água por quilo de peso corporal (padrão e corrida). */
...
export function idadeRepresentativaDaFaixa(valorFaixa: string): number {
  ...
  // Usa o ponto médio da faixa como idade representativa (ex.: "15-19" → 17).
...
export function calcularMetaCalorica(
  ...
  // Aplica o ajuste percentual do objetivo sobre o gasto total.
...
export function distribuirMacros(
  ...
  pesoKg: number,
  objetivo: Objetivo,
): Pick<DistribuicaoMacros, 'proteinas_g' | 'carboidratos_g' | 'gorduras_g' | 'fibras_g' | 'agua_ml'> {
  // 1) Proteína: gramas por quilo definidos pelo objetivo.
  const proteinasGramas = Math.round(pesoKg * PROTEINA_G_POR_KG[objetivo]);
  // 2) Gordura: percentual das calorias totais convertido para gramas.
  const gordurasGramas = Math.round((metaKcal * GORDURA_PERCENTUAL[objetivo]) / KCAL_POR_GRAMAS.gordura);
  // 3) Calorias já ocupadas por proteína e gordura.
  const caloriasDaProteina = proteinasGramas * KCAL_POR_GRAMAS.proteina;
  const caloriasDaGordura = gordurasGramas * KCAL_POR_GRAMAS.gordura;
  // 4) Carboidrato: preenche o restante das calorias da meta.
  const carboidratosGramas = Math.round(
    Math.max(0, metaKcal - caloriasDaProteina - caloriasDaGordura) / KCAL_POR_GRAMAS.carboidrato,
  );
  // 5) Fibras: 14 g por 1000 kcal, limitadas entre 25 g e 40 g diárias.
  const fibrasGramas = Math.min(40, Math.max(25, Math.round(14 * (metaKcal / 1000))));
  // 6) Água: mililitros por quilo conforme o objetivo.
  const aguaMl = Math.round(pesoKg * AGUA_ML_POR_KG[objetivo]);
  return {
...
/** Calcula o plano nutricional COMPLETO para um perfil (ponto de entrada do motor). */
export function calcularPlanoNutricional(perfil: {
  ...
  const macros = distribuirMacros(metaKcal, perfil.peso_kg, perfil.objetivo);
